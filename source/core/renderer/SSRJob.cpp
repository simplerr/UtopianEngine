...
namespace Utopian
{
	...
	void SSRJob::InitFirstPass(const std::vector<BaseJob*>& jobs, const GBuffer& gbuffer)
	{
		...
		mCalculateSSREffect->BindCombinedImage("specularSampler", gbuffer.albedoImage.get(), mCalculateRenderTarget->GetSampler());
		mCalculateSSREffect->BindCombinedImage("normalViewSampler", gbuffer.normalViewImage.get(), mCalculateRenderTarget->GetSampler());
		mCalculateSSREffect->BindCombinedImage("normalWorldSampler", gbuffer.normalImage.get(), mCalculateRenderTarget->GetSampler());
		...
		gScreenQuadUi().AddQuad(100, 100, size, size, ssrImage.get(), mCalculateRenderTarget->GetSampler());
	}
