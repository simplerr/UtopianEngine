                           ...
                           inout bool keep_going)
{
   ...
   if (p_dist < OBSTACLE_JUMP_MIN_DISTANCE) {
      OutputColor = FAIL_COLOR_OBSTACLE;
      keep_going = false;
      return false;
   }
   ...
   for (int i = 0; i < RAY_MARCH_OBSTACLE_MAX_SAMPLES; i++) {
      ...
      if (is_outside_screen(ss_jp.xy)) {
         OutputColor = FAIL_COLOR_NOT_IN_SCREEN;
         keep_going = false;
         return false;
      }
      ...
      if (ss_jp.z >= jp_depth) {
         ...
         if (jp_dist - prev_jp_dist > OBSTACLE_BREAK_DEPTH_DISTANCE) {
            ...
            return true;
#else // case c)
            OutputColor = FAIL_COLOR_DOUBLE_OBSTACLE;
            keep_going = false;
            return false;
#endif
         }
...
get_reflection_position(vec3 ss_refl_dir, vec3 ss_p_min, out vec3 ss_p_out)
{
    ...
    * point.
    */
   OutputColor = FAIL_COLOR_REFLECTION_TOO_FAR;
   return false;
}
...
vec4 retrieveReflectionColor(vec3 worldPosition, vec3 viewNormal, vec2 uv, float reflectiveness)
{
   ...
   if (dp_view_refl < MIN_DOT_REFLECTION_VIEW) {
      OutputColor = FAIL_COLOR_BACK_CAM;
      return;
   ...
   vec3 ss_frag_pos = vec3(uv, getDepth(uv));
